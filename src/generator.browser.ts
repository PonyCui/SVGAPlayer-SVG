      ...
      if (
        ...
        for (const shapeIndex in shapes) {
          ...
          for (let index = 0; index < this.videoItem.frames; index++) {
            ...
            if (
              shapeFrame.shape.d !== undefined &&
              shapeFrame.shape.d !== null &&
              shapeFrame.shape.d.trim().length > 0
            ) {
            ...
            if (shapeFrame.transform !== undefined && shapeFrame.transform !== null) {
              const unmatrix = parseMatrix([
                ...
                shapeFrame.transform.c,
                shapeFrame.transform.d,
                shapeFrame.transform.tx,
                shapeFrame.transform.ty,
              ]);
              animateLayers["translate"].push(
          ...
          for (const attrName in animateLayers) {
            ...
            if (standardValues.every((it) => it === standardValues[0])) {
              if (noValueProps[attrName] === standardValues[0]) {
                continue;
              }
